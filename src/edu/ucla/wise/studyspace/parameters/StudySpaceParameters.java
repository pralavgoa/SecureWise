...
public class StudySpaceParameters {
	...
	public StudySpaceParameters(Map<String, String> parametersMap) {
		...
		this.id = parametersMap
				.get(StudySpaceParametersProvider.STUDY_SPACE_ID);
		this.serverUrl = parametersMap
				.get(StudySpaceParametersProvider.SERVER_URL);
		this.serverApplication = parametersMap
				.get(StudySpaceParametersProvider.SERVER_APPLICATION);
		this.sharedFiles_linkName = parametersMap
				.get(StudySpaceParametersProvider.SHARED_FILES_LINK_NAME);
		this.folderName = parametersMap
				.get(StudySpaceParametersProvider.DIRECTORY_NAME);
		this.databaseName = parametersMap
				.get(StudySpaceParametersProvider.DATABASE_NAME);
		this.databaseUsername = parametersMap
				.get(StudySpaceParametersProvider.DATABASE_USER);
		this.databasePassword = parametersMap
				.get(StudySpaceParametersProvider.DATABASE_PASSWORD);
		this.projectTitle = parametersMap
				.get(StudySpaceParametersProvider.PROJECT_TITLE);
		this.databaseEncryptionKey = parametersMap
				.get(StudySpaceParametersProvider.DATABASE_CRYPTIC_KEY);
		this.emailSendingTime = parametersMap
				.get(StudySpaceParametersProvider.EMAIL_SENDING_TIME);

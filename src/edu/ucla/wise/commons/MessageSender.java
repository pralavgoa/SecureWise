...
public class MessageSender {
    ...
    public MessageSender() {
    	session = WISEApplication.getMailSession(null);
    }

    /**
     * Constructor : gets the email session from WISE Application 
     * and other instance variables from MessageSequence.
     * 
     * @param	msgSeq	Message sequence for which sender has to be created.
     */
    public MessageSender(MessageSequence msgSeq) {
    	...
    	session = WISEApplication.getMailSession(null);
    	fromStr = msgSeq.getFromString();
    	replyStr = msgSeq.getReplyString();
    }
     ...
     * 
     * @param 	msg				Message object to be sent to the User.
     * @param 	messageUseID	The id used for generating URL which is
     * 								unique to the user.
     * @param 	toUser			User to whom the message has to be sent.
     * @param 	db				Data bank object used from talking to DB.
     ...
     * @param 	messageUseID	The id used for generating URL which is
     * 								unique to the user.
     * @param 	toEmail			Email id to which message has to be sent.
     ...
     * @param 	db				Data bank object used from talking to DB.
     * @param 	inviteeId		Id of the user to whom email has to be sent
     * @return  String			Empty string if successful.
     */
    public String sendMessage(Message msg, String messageUseID,
    		String toEmail, String salutation, String lastname, String ssid, DataBank db, String inviteeId) {
    	...
    	try {
    		...
    		InternetAddress tmpAddr = new InternetAddress(fromStr);
    		mMessage.setFrom(tmpAddr);
    		if (replyStr != null) {
    			tmpAddr = new InternetAddress(replyStr);
    		...
    		if (message == null) {
    			...
    			/* create an "Alternative" Multipart message to send both html & text email */
    			Multipart mp = new MimeMultipart("alternative");
    			
    			/* add text body part */
    			BodyPart bpText = new MimeBodyPart();
    			bpText.setDataHandler(new DataHandler(msg.composeTextBody(
    					salutation, lastname, ssid, messageUseID),
    					"text/plain"));
    		...
    		outputString = mailingProcess(mMessage);
   ...
   /* 
   //Overloaded method for send_message
    public String send_message(Message msg, String invitee_id, String message_seq_id, String toEmail, String salutation, String lastname, 
    		String ssid, Data_Bank dataBank, String msg_type, String survey_id) {
		String outputString = "uncaught exception";
		String message = null;
		
		try {
			Connection conn = dataBank.getDBConnection();
			...
			//Email sending is done, need to update the tables
			if (outputString.equalsIgnoreCase("")){
			//updating the survey_message_use table with the sucess status
			 String msgUse_sql = "INSERT INTO survey_message_use (messageId,invitee, survey, message) VALUES('"
		    + message_useID + "', "
		    + invitee_id + ", '"
		    + survey_id + "', '" 
			+ msg.id + "')";
			AdminInfo.log_info("The sql query run when inserting into survey_message_use "+ msgUse_sql);
			msgUseQry.execute(msgUse_sql);
			String state = "invited";
			outputString=message_useID;
			//updating the survey_user_state table
		    ...
		    String sql_u = "INSERT INTO survey_user_state (invitee, state, survey, message_sequence) "
				...
				AdminInfo.log_info("The sql query run when inserting into survey_user_state "+ msgUse_sql);
     ...
     * 
     * @param 	msgText		Message test that has to be sent.
    ...
    public String sendTest(String msgText) {
    	...
    	try {
    		...
    		String msgResult = mailingProcess(message);
     ...
     * 
     * @param 	msg		Mime message which contains the details for sending email.
    ...
    public String mailingProcess(MimeMessage msg) throws MessagingException,
    	...
    	try {
    		...
    		else{Transport.send(msg, msg.getAllRecipients());}
    		tr.close();

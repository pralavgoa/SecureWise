		...
		<%
			...
			if (inv != null) {
				if (session.getAttribute("EditType") != null) {
					...
					inv[0].userName = (String) request
							.getParameter("username_" + inv[0].id);
					inv[0].firstName = (String) request.getParameter(
							"firstname_" + inv[0].id).toLowerCase();
					inv[0].lastName = (String) request.getParameter(
					...
					inv[0].email = (String) request.getParameter("email_"
							+ inv[0].id);
					newId = InterviewManager.getInstance().addInterviewer(
							adminUserSession.getMyStudySpace(), inv[0]);
					//remove the session attributes
					session.removeAttribute("EditType");

				} else {
					for (int i = 0; i < inv.length; i++) {
						inv[i].userName = (String) request
								.getParameter("username_" + inv[i].id);
						inv[i].firstName = (String) request.getParameter(
								"firstname_" + inv[i].id).toLowerCase();
						inv[i].lastName = (String) request.getParameter(
						...
						newId = InterviewManager.getInstance().saveProfile(
								adminUserSession.getMyStudySpace(), inv[i]);

...
public abstract class PageItem {
    ...
    public String html = "";
    public Condition cond = null; // any page item can be conditional on previous responses

    /**
     * Checks if the given DOM node is a Page Item or not based on the question type.
     *  
     * @param 	n		DOM Node whose validated is checked.
    ...
    public static boolean IsPageItemNode(Node n) {
		...
		try {
		    ...
		    WISEApplication.logError("PAGE ITEM test attempt failed for " + n
		    		+ ": " + e, null);
		}
		return answer;
    }
     ...
     * subclasses.
     *  
     * @param 	n			DOM Node whose hierarchy is checked.
     * @return 	PageItem	Appropriate Item is returned based on the type of question.
     */
    public static PageItem MakeNewItem(Node n) {
		...
		try {
		    ...
		    if (nname.equalsIgnoreCase("Open_Question")) {
		    	...
		    	WISEApplication.logError("PAGE ITEM Creation attempt failed for "
			    		+ nname, null);
		    }
		} catch (NullPointerException e) {
		    WISEApplication.logError("PAGE ITEM Creation attempt failed for "
     ...
     * from the DOM node.
     * 
     * @param n		DOM node from where the page item is populated. 
     */
    public PageItem(Node n) {
		try {
		    ...
		    WISEApplication
			    .logError("PAGE ITEM ROOT CONSTRUCTOR: " + e, null);
		    return;
		}  catch (NullPointerException e) {
			 WISEApplication
			    .logError("PAGE ITEM ROOT CONSTRUCTOR: " + e, null);
     ...
     * 
     * @param mySurvey 	Current running survey. 
     */
    public void knitRefs(Survey mySurvey) {
		try {
		    throw new Exception("knitRefs called on " + itemType + " " + name);
		} catch (Exception e) {
		    WISEApplication.logError("Unimplemented PageItem method: " + e,
		    		null);
		}
    }
     ...
     * 
     * @param String[]	Return null always. 
     */
    public String[] listFieldNames() {
		try {
		    throw new Exception("listFieldNames() called on " + itemType + " "
		    		+ name);
		} catch (Exception e) {
		    WISEApplication.logError("Unimplemented Page_item method: " + e,
		    		null);
		}
		return null;
    }
    ...
    /** 
     * Returns the results of survey in form of string -- be overwritten by subclass.
     * 
    ...
    public Hashtable<String, String> readForm(Hashtable<String, String> params) {
		try {
		    throw new Exception("read_form called on " + itemType + " " + name);
		} catch (Exception e) {
		    WISEApplication.logError("Unimplemented Page_item method: " + e,
			    null);
		}
     ...
     * 
     * @param	User			The user whose pre-conditions are checked.
     * @param	elementNumber	Element number of the question.
    ...
    public String renderForm(User theUser, int elementNumber) {
		...
		if (cond != null) {
		    ...
		    pageItemHtml.append(cond.getJsExpression().toUpperCase());
		...
		if (cond != null) {
		    ...
		    boolean writeQuestion = cond.checkCondition(theUser);
		    /* 
		     * if it doesn't meet the precondition, skip writing this question

	...
	public WISEApplication(WiseProperties properties){
		...
		try {
			...
			if (Strings.isNullOrEmpty(wiseProperties.getXmlRootPath())) {
				log.error("WISE Application initialization Error: Failed to read from Shared properties file "
						+ sharedPropPath + "\n");
			}
			...
			if (mailSession == null) {
				log.error("WISE Application initialization Error: Failed to initialize mail session\n");
			}

...
public class Message {
	...
	try {
	    ...
	    this.subject = n.getAttributes().getNamedItem("Subject")
		    .getNodeValue();
		...
		if (nodeP.item(j).getNodeName().equalsIgnoreCase("Message_Ref")
			&& !hasRef) {

		    /* check prevents a 2nd ref (that's invalid) */
		    this.msgRef = nodeP.item(j).getAttributes()
			    .getNamedItem("ID").getNodeValue();
		    hasRef = true;
		    break;
		} else {
		    if (nodeP.item(j).getNodeName().equalsIgnoreCase("p")) {
			this.mainBody += nodeP.item(j).getFirstChild()
				.getNodeValue()
				+ "\n\n";
			this.htmlBody += "<p>"
			...
			LOGGER.info("########The Body of the email is: "
				+ this.mainBody);
	    ...
	    LOGGER.error("WISE - TYPE MESSAGE: ID = " + this.id
		    + "; Subject = " + this.subject + " --> " + e.toString(), e);
	    return;
	}
    }
	...
	if (this.htmlFormat) {
	    ...
	    this.htmlTail = "</font></td><td width=20>&nbsp;</td>"
		    + "</tr></table></td></tr>"
		    + "<tr><td>"
		    + "<table width=100% border=0 cellpadding=0 cellspacing=0>"
	...
	if (this.hasLink) {
	    String reminderLink = this.servletPath + "survey?msg=" + msgIndex
		    + "&t=" + WISEApplication.encode(ssid);

	    String declineLink = this.servletPath + "survey/declineNOW?m="
		    + msgIndex + "&t=" + WISEApplication.encode(ssid);
	...
	return servletPath + "survey?msg=" + msgIndex + "&t="
		+ WISEApplication.encode(surveyId);
    }
	...
	if (this.hasLink) {
	    String reminderLink = this.servletPath + "survey?msg=" + msgIndex
		    + "&t=" + WISEApplication.encode(ssid);
	    String declineLink = this.servletPath + "declineNOW?m=" + msgIndex
		    + "&t=" + WISEApplication.encode(ssid);

...
public class RepeatingItemSet extends PageItem {
    ...
    /**
     * Parses a Repeating Question set from survey file and 
     * constructs the RepeatingItemSet Object
     * 
     * @param iNode	XML node from which all the details are obtained.
     */
		    ...
		    for (int i = 0; i < nodeList.getLength(); i++) {
				...
				if (PageItem.IsPageItemNode(childNode)) {
				    ...
				    if (childNode.getNodeName().equalsIgnoreCase(
				    		"Precondition")) {
				    	preCondition = new Condition(childNode);
				    }
				}
		    }
    ...
    /**
     * Renders the repeating item set question as HTML this 
     * used while the survey is being displayed for the user.
     * 
     * @param iUser		The user for whom this question is being displayed.
     * @param itemIndex	Used for the assigning the ID for the repeating Item 
     * 					set question in the html.
     * @return
     */
    public String renderRepeatingItemSet(User iUser, int itemIndex) {
		...
		if (preCondition != null) {
		    ...
		    htmlContent.append(cond.getJsExpression().toUpperCase());	
		    htmlContent.append(";");
		    htmlContent.append("};");
		    htmlContent.append("</script>");
		}
		...
		if (preCondition != null) {
		    ...
		    boolean writeQuestion = cond.checkCondition(iUser);
		...
		for (int i = 0; i < itemSet.size(); i++) {
		    htmlContent.append(itemSet.get(i).renderForm(iUser,
			    100 * itemIndex + i));// 100 is multiplied to get diff div id, not good
		}
		htmlContent
    ...
    public int countFields() {
		int fieldCount = 0;
		for (PageItem repeatingItem : itemSet) {
    ...
    public String renderForm(User iUser, int itemIndex) {
    	return renderRepeatingItemSet(iUser, itemIndex);
    }
     ...
     * 
     * @return Array	Array of string that contains all the field name for 
     * 					a repeating item set question.
     */
    @Override
     ...
     * 
     * @param 	n					XML node from the survey for the repeating item set.
     * @return	RepeatingItemSet	Newly created repeating item set of the XML node is returned
     */
    ...
    /**
     * Returns of the DOM node passed corresponds to a repeating item set or not.
     * 
     * @param 	n			DOM node from the survey to check for the repeating item set.

...
public class AppImageRenderServlet extends HttpServlet {
    ...
    private static Logger LOGGER = Logger
	    .getLogger(AppImageRenderServlet.class);
	...
	if (SanityCheck.sanityCheck(imageName)) {
	    response.sendRedirect(WISEApplication.rootURL + "/WISE/"
		    + WiseConstants.ADMIN_APP + "/sanity_error.html");
	    return;
	}
	...
	try {
	    ...
	    String pathToImages = WISEApplication.wiseProperties
		    .getImagesPath();
	    ...
	    e.printStackTrace();
	    LOGGER.error(
		    "Error while reading file from file system due to null buffer",
		    e);
	} catch (IndexOutOfBoundsException e) {
	    e.printStackTrace();
	    LOGGER.error(
		    "Error while reading file from file system due to illegal buffer lengths",
		    e);
	} finally {
	    if (imageStream != null) {
		try {

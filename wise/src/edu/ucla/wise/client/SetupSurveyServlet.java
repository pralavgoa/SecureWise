...
public class SetupSurveyServlet extends HttpServlet {
	...
	if (theUser.getCurrentPage() != null) {
	    ...
	    parametersForSurveyPage.put("sharedUrl", SurveyorApplication
		    .getInstance().getSharedFileUrl());
	    parametersForSurveyPage.put("userId", theUser.getId());
	    parametersForSurveyPage.put("pageHtml", this.getPageHTML(theUser));
	    parametersForSurveyPage.put("progressBarHtml",
		    this.getProgressDivContent(theUser, session));
	    try {
		out.print(getHtmlFromTemplate(parametersForSurveyPage));
    ...
    public static String getHtmlFromTemplate(Map<String, Object> mapOfParameters)
	    throws IOException, TemplateException {
	Configuration cfg = SurveyorApplication.getInstance()
		.getHtmlTemplateConfiguration();
    ...
    public static String getHtml(String userId, String pageHtml,
	    String progressDivHtml) {
	...
	if ((pOutput != null) && !pOutput.equalsIgnoreCase("")) {
	    ...
	    if ((theUser.getCurrentSurvey()
		    .isLastPage(theUser.getCurrentPage().id))
		...
		if (theUser.getCurrentPage().nextPage.equalsIgnoreCase("NONE")) {
		    pageHtml.append("<input type='hidden' name='nextPage' value='"
			    + theUser.getCurrentSurvey().nextPage(
				    theUser.getCurrentPage().id).id + "'>");
		} else {

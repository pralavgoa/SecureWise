...
public class StudySpaceParametersAcceptor extends HttpServlet {
    ...
    public void service(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        if (Strings.isNullOrEmpty(serverURL) || Strings.isNullOrEmpty(serverAppName)
                || Strings.isNullOrEmpty(serverSharedLinkName) || Strings.isNullOrEmpty(directoryName)
                || Strings.isNullOrEmpty(databaseUsername) || Strings.isNullOrEmpty(databasePassword)
                || Strings.isNullOrEmpty(projectTitle) || Strings.isNullOrEmpty(databaseEncryptionKey)) {
            out.write("<div>Please ensure that none of the parameters are empty</div>");
            out.write("<div>" + studySpaceName + serverURL + serverAppName + serverSharedLinkName + directoryName
                    + databaseUsername + databasePassword + projectTitle + databaseEncryptionKey + "</div>");
            return;
        }
        ...
        if (StudySpaceCreator.createStudySpace(studySpaceName, databasePassword,
            ...
            if (databaseConnector.writeStudySpaceParams(studySpaceName, serverURL, serverAppName, serverSharedLinkName,
                    directoryName, databaseUsername, databaseName, databasePassword, projectTitle,

package edu.ucla.wise.persistence.data;

public class DBConstants {
    public static final String SURVEY_USER_PAGE_STATUS_TABLE = "survey_user_page";
    public static final String MAIN_DATA_TEXT_TABLE = "data_text";
    public static final String MAIN_DATA_INTEGER_TABLE = "data_integer";
    public static final String INVITEE_TABLE = "invitee";
    public static final String DATA_RPT_INS_TO_QUES_ID_TABLE = "data_rpt_ins_id_to_ques_id";
    public static final String DATA_REPEAT_SET_INSTANCE_TABLE = "data_repeat_set_instance";

}

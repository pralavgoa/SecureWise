...
public class Survey {
	...
	if (this.getInviteeMetadata() == null) {
	    throw new IllegalStateException(
		    "Please provide the invitee metadata in the survey xml");
	}
	this.inviteeFields = new String[this.getInviteeMetadata().fieldMap
		.size()];
	int cnt = 0;
	for (Map.Entry<String, Values> map : this.getInviteeMetadata().fieldMap
		.entrySet()) {
	...
	for (int i = 0; i < nodelist.getLength(); i++) {
		...
		for (int j = 0, k = 0; j < nodelist1.getLength(); j++) {
			...
			this.totalItemCount += this.getPages()[k]
				.getItemCount();
			k++;
		    }
		}
	...
	try {
	    ...
	    s += "<LINK href='" + "styleRender?app="
		    + this.getStudySpace().studyName + "&css=style.css"
	    ...
	    s += "<img src='" + "imageRender?app="
		    + this.getStudySpace().studyName + "&img=" + this.logoName
		    + "' border=0></td></tr>";
	    s += "<tr><td>&nbsp;</td></tr>";
	...
	try {
		...
		if (i != idx) {
		    ...
		    s += "<font size=-2><b>" + this.getPages()[i].title
			    + "</b></font></a>";
		} else {
		    s += "<td bgcolor='#FFFF00' align=left colspan=1>";
		    s += "<font size=-2><b>" + this.getPages()[i].title

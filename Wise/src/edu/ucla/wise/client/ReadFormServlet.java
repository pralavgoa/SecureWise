...
public class ReadFormServlet extends HttpServlet {
    private static final Logger LOGGER = Logger
	    .getLogger(ReadFormServlet.class);
	...
	if (action.equalsIgnoreCase("linkpage")) {
	    ...
	    session.invalidate();
	    newPage = SurveyorApplication.getInstance().getSharedFileUrl()
		    + "interrupt" + SurveyorApplication.htmlExt;
	    ...
	    session.invalidate();
	    newPage = SurveyorApplication.getInstance().getSharedFileUrl()
		    + "timeout" + SurveyorApplication.htmlExt;
	    out.println(this.pageReplaceHtml(newPage));
	    return;
	...
	if (theUser.completedSurvey()) {
		...
		if ((theUser.getCurrentSurvey().getForwardUrl() != null)
			...
			newPage = newPage
				...
				+ "/survey?t="
				+ WISEApplication.encode(theUser
					.getCurrentSurvey().getEduModule())
				+ "&r="
			...
			newPage = newPage
				...
				+ "&ss="
				+ WISEApplication.encode(theUser
					.getCurrentSurvey().getStudySpace().id);
			...
			newPage = SurveyorApplication.getInstance()
			...
			if (currentNumbCompleters >= theUser.getCurrentSurvey()
				.getMinCompleters()) {
			    review = "view_results";
			}

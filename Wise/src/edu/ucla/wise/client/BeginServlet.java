...
public class BeginServlet extends HttpServlet {
	...
	if (Strings.isNullOrEmpty(msgId)) {
		...
		res.sendRedirect(sharedFileUrl + "link_error"
			+ edu.ucla.wise.commons.SurveyorApplication.htmlExt);
		return;
	    }
	...
	if ((theUser == null) || (theUser.getId() == null)) {
	    req.getRequestDispatcher(
		    "./survey/error_jsps/UserNotIdentified.jsp?studySpaceName="
			    + theStudy.studyName).forward(req, res);
	    LOGGER.error("WISE Error: Begin servlet failed for message id "
		    + msgId, null);
	    return;
	}
	...
	if ((sharedFileUrl != null) || (sharedFileUrl.length() != 0)) {
	    mainUrl = "" + sharedFileUrl + "browser_check"
		    + edu.ucla.wise.commons.SurveyorApplication.htmlExt + "?w="
	...
	out.println("<frame name='mainFrame' src='" + sharedFileUrl
		+ "error_javascript.htm'>");
	out.println("</frameset><noframes></noframes></HTML>");
	out.close();
    }

    private HttpSession createNewSessionForAnonymousUser(

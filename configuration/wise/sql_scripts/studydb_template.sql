...
CREATE TABLE `survey_user_session` (
  ...
  `from_message` varchar(64) NOT NULL,
...
--
-- Triggers for the Auditlogs
--
CREATE TRIGGER Insert_Audit
AFTER INSERT ON survey_user_session
FOR EACH ROW 
INSERT INTO audit_logs (messageId,invitee,firstname,lastname,patient_name,ipAddress,actions,updated_time) SELECT NEW.from_message,invitee.id,invitee.firstname,invitee.lastname,invitee.ptname,NEW.ip_address,"Survey Accessed",now() FROM invitee,survey_message_use WHERE (survey_message_use.messageId=NEW.from_message) AND (survey_message_use.invitee = invitee.id);

CREATE TRIGGER Update_Audit
AFTER UPDATE ON survey_user_state
FOR EACH ROW 
UPDATE audit_logs SET actions= CONCAT('Survey ',NEW.state) 
WHERE audit_logs.invitee=NEW.invitee AND audit_logs.updated_time = (SELECT MAX(updated_time) FROM (SELECT * FROM audit_logs) as temp WHERE temp.invitee=NEW.invitee)


/*
CREATE TRIGGER Update_Audit
AFTER UPDATE ON survey_user_session
FOR EACH ROW 
UPDATE audit_logs SET actions= (SELECT CONCAT('Survey has been ',state) FROM survey_message_use,survey_user_state WHERE (survey_message_use.messageId=NEW.from_message) AND (survey_message_use.invitee = survey_user_state.invitee))
WHERE messageId=NEW.from_message;

CREATE TRIGGER Update_Audit
AFTER UPDATE ON survey_user_state
FOR EACH ROW 
CREATE VIEW temp AS SELECT MAX(updated_time) FROM audit_logs WHERE audit_logs.invitee=NEW.invitee;
UPDATE audit_logs SET actions= CONCAT('Survey has been ',NEW.state) 
WHERE audit_logs.invitee=NEW.invitee AND audit_logs.updated_time = temp.updated_time;

CREATE TRIGGER Update_Audit
AFTER UPDATE ON survey_user_state
FOR EACH ROW 
UPDATE audit_logs SET actions= CONCAT('Survey has been ',NEW.state) 
WHERE audit_logs.invitee=NEW.invitee AND audit_logs.updated_time = (SELECT MAX(updated_time) FROM audit_logs WHERE audit_logs.invitee=NEW.invitee)
*/
